...
def send_issue_email(issue_data: dict, image_path: str | None = None, user_email: str | None = None):
    # Disabled backend SMTP email.
    # On Free Cloud tiers (Render, Vercel, GCP), outbound SMTP ports (465, 587, 25) are heavily restricted.
    # This leads to "[Errno 101] Network is unreachable" errors.
    # Since we successfully migrated our email functionality to the frontend using EmailJS (HTTP API Port 443),
    # this duplicate email logic is no longer needed.
    pass
...
def get_nearby_pois(lat: float, lng: float, radius: int = 500) -> list:
    ...
    """
    try:
        response = req.post(overpass_url, data={'data': overpass_query}, timeout=15)
